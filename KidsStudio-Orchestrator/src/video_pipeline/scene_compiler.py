...
def compile_dynamic_scene_sequence(project_manifest_path: str, output_scratch_dir: str = "./scratch/render_output"):
    ...
    for scene in manifest["timeline_scenes"]:
        ...
        for speaker in speakers_in_scene:
            # Look up standard lowercase speaker ID in manifest characters list
            speaker_folder = None
            for char_spec in characters:
                folder = char_spec["folder_name"]
                if speaker.lower() in folder:
            ...
            print(f"🎬 Rhubarb Sync: Generated timing sheet for [{speaker}]")
        ...
        for char_spec in characters:
            ...
            # Map mouth anchor offset (offset center by 100px)
            raw_mx, raw_my = bundle["mouth_anchor_xy"]
            mouth_pos = (raw_mx - 100, raw_my - 100)
        ...
        for frame_idx in range(total_frames):
            ...
            for puppet in active_puppets:
                ...
                active_shape = "X"
                speaker_id = "peacock" if "peacock" in puppet["folder"] else "kalu"
                ...
                character_assets = {
                    "body": puppet["bundle"]["body"],
                    "feathers": puppet["bundle"]["feathers"],
                    "mouths": puppet["bundle"]["mouths"],
                    "mouth_anchor_coordinates": puppet["mouth_pos"],
                ...
                # Calculate movement coordinate interpolation
                ...
                orig_w, orig_h = char_frame.size
                current_w = int(puppet["scale_factor"] * curr_scale)
                current_h = int(orig_h * (current_w / orig_w))
                
                char_resized = char_frame.resize((current_w, current_h), Image.Resampling.LANCZOS)
                
                # Paste
                frame_canvas.paste(char_resized, box=(int(curr_x), int(curr_y)), mask=char_resized)
    ...
    final_output = output_final_dir / "ghamandi_mor_final.mp4"
