...
def test_gen():
    ...
    # Try different model parameter values on the free endpoint
    test_urls = {
        "default": f"https://image.pollinations.ai/prompt/{encoded}?width=1024&height=576",
        "flux": f"https://image.pollinations.ai/prompt/{encoded}?width=1024&height=576&model=flux",
        "turbo": f"https://image.pollinations.ai/prompt/{encoded}?width=1024&height=576&model=turbo",
        "sana": f"https://image.pollinations.ai/prompt/{encoded}?width=1024&height=576&model=sana",
        "default_no_nologo": f"https://image.pollinations.ai/prompt/{encoded}?width=1024&height=576&nologo=false&private=false",
    }
    ...
    for name, url in test_urls.items():
        ...
        try:
            ...
            if r.status_code == 200:
                out_path = Path(f"/Users/lalitprasadsingh/.gemini/antigravity/scratch/content-automation-pipeline/test_free_{name}.png")
